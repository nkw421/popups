      ...
      <div style={{ padding: "28px" }}>
        ...
        >
          ...
          {[
            ...
            { l: "세션 수", v: `${item.sessions}회` },
            { l: "등록 인원", v: `${item.enrolled}명` },
          ].map((r) => (
            <div
              key={r.l}
              style={{
...
export default function ProgramManage({ subTab = "all" }) {
  const [items, setItems] = useState(() =>
    DATA.programs.map((e) => ({ ...e, _visible: true })),
  );
  ...
  const rows = items.filter((e) => e._visible).filter(filterFn);
  const showToast = (msg, type = "success") => setToast({ msg, type });

  /* 통계 */
  const vis = items.filter((e) => e._visible);
  const totalPrograms = vis.length;
  const activePrograms = vis.filter((e) => e.status === "active").length;
  const totalEnrolled = vis.reduce((a, b) => a + b.enrolled, 0);
  const pendingPrograms = vis.filter((e) => e.status === "pending").length;
  ...
  const handleCreate = (f) => {
    setItems((p) => [
      {
        ...f,
        id: `PG-${String(p.length + 1).padStart(3, "0")}`,
        _visible: true,
      },
      ...p,
    ]);
  ...
  const handleBulkDelete = () => {
    ...
    setSelected(new Set());
    showToast(`${ids.size}건의 프로그램이 삭제되었습니다.`);
  };
  ...
  return (
    <div>
      ...
      >
        ...
        {rows.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            <Clipboard size={36} color="#CBD5E1" style={{ marginBottom: 12 }} />
            <div
              style={{
                fontSize: 14,
                fontWeight: 600,
      ...
      {modal?.type === "deleteAll" && (
        <ConfirmModal
          title="전체 프로그램 삭제"
          msg={`현재 필터의 ${rows.length}건 프로그램을 모두 삭제하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.`}
