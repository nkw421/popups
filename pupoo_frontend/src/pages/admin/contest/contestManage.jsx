      ...
      <div style={{ padding: "28px" }}>
        ...
        >
          ...
          {[
            ...
            { l: "참가팀", v: `${item.teams}팀` },
            { l: "투표수", v: `${item.votes.toLocaleString()}표` },
            { l: "상금", v: item.prize },
            { l: "일정", v: item.date },
          ].map((r) => (
            <div
...
export default function ContestManage() {
  const [items, setItems] = useState(() =>
    DATA.contestManage.map((e) => ({ ...e, _visible: true })),
  );
  ...
  const activeContests = rows.filter((e) => e.status === "active").length;
  const totalTeams = rows.reduce((a, b) => a + b.teams, 0);
  ...
  const handleCreate = (f) => {
    setItems((p) => [
      {
        ...f,
        id: `CT-${String(p.length + 1).padStart(3, "0")}`,
        teams: 0,
        votes: 0,
        _visible: true,
      },
  ...
  const handleBulkDelete = () => {
    ...
    setSelected(new Set());
    showToast(`${ids.size}건의 콘테스트가 삭제되었습니다.`);
  };
  ...
  return (
    <div>
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            >
              <Plus size={13} strokeWidth={2.5} /> 콘테스트 등록
            </button>
          </div>
        </div>
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid #F1F5F9" }}>
              ...
              {[
                { label: "콘테스트명", w: "24%" },
                { label: "카테고리", w: 90 },
                { label: "참가팀", w: 70, align: "right" },
                { label: "투표수", w: 80, align: "right" },
                { label: "상금", w: 90 },
                { label: "일정", w: 100 },
                { label: "상태", w: 72 },
                { label: "", w: 150 },
        ...
        {rows.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            <Trophy size={36} color="#CBD5E1" style={{ marginBottom: 12 }} />
            <div
              style={{
                fontSize: 14,
                fontWeight: 600,
      ...
      {modal?.type === "delete" && (
        <ConfirmModal
          title="콘테스트 삭제"
          msg={`"${modal.item.name}" 콘테스트를 삭제하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.`}
      ...
      {modal?.type === "bulkDelete" && (
        <ConfirmModal
          title="선택 콘테스트 삭제"
          msg={`선택한 ${selected.size}건의 콘테스트를 삭제하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.`}
      ...
      {modal?.type === "deleteAll" && (
        <ConfirmModal
          title="전체 콘테스트 삭제"
          msg={`현재 목록의 ${rows.length}건 콘테스트를 모두 삭제하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.`}
