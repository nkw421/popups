            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <div
                ...
                style={{
                  minHeight: 100,
                  borderRadius: 14,
                  border: `2px dashed ${dragOver ? "#5D8C2A" : "#A8D070"}`,
                  background: dragOver ? "#F4F9EC" : "#f8fafc",
                  padding: 20,
                  cursor: "pointer",
                  display: "flex",
              ...
              >
                <Upload size={24} color="#5D8C2A" />
                <span style={{ fontSize: 14, fontWeight: 700, color: "#5D8C2A" }}>
                  ...
                  {files.map((file, index) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        overflow: "hidden",
                        border: dragIndex === index ? "2px solid #5D8C2A" : dropIndex === index ? "2px solid #A8D070" : "1px solid #e2e8f0",
                        background: "#fff",
                    ...
                    >
                      <div style={{ aspectRatio: "1", position: "relative", background: "#f1f5f9" }}>
                        ...
                        {index === 0 ? (
                          <span style={{ position: "absolute", top: 6, left: 6, padding: "4px 8px", borderRadius: 6, background: "#5D8C2A", color: "#fff", fontSize: 11, fontWeight: 800 }}>
                            대표
                          </span>
                        ) : null}
                        <button
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ ...modalShellStyle("min(1060px, 100%)"), display: "flex", flexDirection: "row", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, maxHeight: "90vh", overflow: "auto" }}>
            ...
            <div style={{ padding: "12px 28px 28px", flex: 1, display: "flex", flexDirection: "column" }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#5D8C2A", marginBottom: 12 }}>{eventName}</div>
...
export default function EventGallery() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <PageHeader title="행사 갤러리" subtitle="실제 행사별 사진을 모아 보고 조회순, 좋아요순, 최신순으로 정렬할 수 있습니다" icon={<Images size={42} color="#7CB342" strokeWidth={1.6} />} titleStyle={{ fontSize: 46, lineHeight: "66px", letterSpacing: "-1px" }} subtitleStyle={{ fontSize: 20 }} categories={SERVICE_CATEGORIES} currentPath="/gallery/eventgallery" onNavigate={(path) => navigate(path)} />
      <main
        style={{
      ...
      >
        <section>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, marginBottom: 24 }}>
            ...
            {isAuthed ? <button type="button" onClick={() => { setWriteError(""); setWriteForm({ eventId: selectedEventId || "", title: "", description: "", files: [] }); setWriteOpen(true); }} style={{ height: 42, padding: "0 18px", borderRadius: 999, border: "none", background: "#5D8C2A", color: "#fff", display: "inline-flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 800, cursor: "pointer", flexShrink: 0 }}><Plus size={16} /> 글쓰기</button> : null}
          </div>
          {reportNotice ? (
