...
export default function GoogleJoin() {
  ...
  const canSendOtp = !loading && step === STEP.FORM && !!providerUid && !!emailTrim && phoneDigits.length >= 10;
  const canVerify = !loading && step === STEP.OTP && !!signupKey && phoneDigits.length >= 10 && (otpCode || "").trim().length >= 4;

  useEffect(() => {
    if (didInitRef.current) return;
    didInitRef.current = true;
    if (!providerUid) { navigate("/auth/login", { replace: true }); return; }
    setStep(STEP.FORM);
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      if (!key) { setError("signupKey가 없습니다. 응답 구조 확인 필요"); return; }
      setSignupKey(key);
      setStep(STEP.OTP);
      if (res?.devOtp) setOtpCode(String(res.devOtp));
    } catch (e) {
      console.error("[GoogleJoin] signupStart error", e?.response?.status, e?.response?.data, e?.message);
  ...
  const verifyOtpAndComplete = async () => {
    ...
    setError(""); setLoading(true);
    try {
      await authApi.signupVerifyOtp({ signupKey, phone: phoneDigits, otpCode: (otpCode || "").trim() });
      const res = await authApi.signupComplete({ signupKey });
      const accessToken = res?.accessToken;
      if (!accessToken) { setError("회원가입 완료 응답에 accessToken이 없습니다."); return; }
      tokenStore.setAccess(accessToken);
      login();
      ["google_temp_password", "google_provider_uid", "google_email", "google_nickname", "post_login_redirect"]
  ...
  const styles = `
    ...
    .gj-input {
      ...
      border: 1.5px solid #e5e5e5; background: #f8f9fc;
      font-size: 16px; font-weight: 500; color: #191919; outline: none;
    ...
    @media (max-width: 480px) {
      .gj-card { padding: 32px 20px 28px; border-radius: 20px; max-width: 100%; }
      .gj-title { font-size: 22px; } .gj-subtitle { font-size: 14px; margin-bottom: 24px; }
      .gj-input { height: 50px; font-size: 15px; padding: 0 16px; }
      .gj-btn-primary { height: 52px; font-size: 16px; } .gj-btn-secondary { height: 48px; font-size: 14px; }
    }
  `;
  ...
  if (step === STEP.INIT) {
    return (
      <>
        <style>{styles}</style>
        <div className="gj-root"><div className="gj-card"><div className="gj-loading-init">구글 인증 확인 중...</div></div></div>
      </>
    );
  }
      ...
      <div className="gj-root">
        <div className="gj-card">
          ...
          <p className="gj-subtitle">
            {step === STEP.FORM ? "추가 정보를 입력하면 가입이 완료됩니다" : "인증번호를 입력해주세요"}
          </p>
          ...
          {step === STEP.FORM && (
            <>
              <div className="gj-field">
                <label className="gj-label">이메일 {hasGoogleEmail && <span style={{ color: "#999", fontWeight: 400 }}>(구글 연동)</span>}</label>
                <input
                  className="gj-input" value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="example@gmail.com"
                  disabled={loading || hasGoogleEmail} type="email"
                />
              ...
              <div className="gj-field">
                ...
                <input className="gj-input" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="01012345678" disabled={loading} inputMode="tel" />
          ...
          {step === STEP.OTP && (
            <>
              ...
              <div className="gj-field">
                ...
                <input
                  className="gj-input" value={otpCode}
                  onChange={(e) => setOtpCode(e.target.value.replace(/[^0-9]/g, ""))}
                  placeholder="6자리 숫자 입력" maxLength={6} inputMode="numeric"
