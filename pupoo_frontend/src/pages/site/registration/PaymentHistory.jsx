    ...
    <div style={{ background: "#f5f6fa", minHeight: "100vh" }}>
      ...
      <main style={{ maxWidth: 960, margin: "0 auto", padding: "20px 16px 80px" }}>
        ...
        >
          ...
          <StatCard
            label="결제 완료 금액"
            value={`${stats.approvedAmount.toLocaleString()}원`}
          />
        </div>

        {!isBootstrapped && (
          <PanelMessage text="세션 확인 중입니다..." />
        )}
        ...
        {isBootstrapped && isAuthed && loading && (
          <PanelMessage text="결제 내역을 불러오는 중입니다..." />
        )}

        {isBootstrapped && isAuthed && !loading && error && (
          <PanelMessage text={error} error />
        )}
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#6b7280" }}>
                      결제일시: {toDateText(item.requestedAt)}
                    </div>
                  </div>
