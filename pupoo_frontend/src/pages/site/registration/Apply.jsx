...
function ApplyForm() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (status === 401) {
        ...
        setErrorMsg("이미 해당 행사에 신청하셨습니다.");
      } else {
        setErrorMsg(msg || "참가 신청에 실패했습니다. 다시 시도해주세요.");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const selectedEventObj = apiEvents.find((e) => e.eventId === selectedEvent);
              ...
              {loadingEvents ? (
                ...
                apiEvents.map((ev) => {
                  ...
                  return (
                    <div
                      key={ev.eventId}
                      className={`reg-event-item${selectedEvent === ev.eventId ? " selected" : ""}`}
                    ...
                    >
                      ...
                      <div className="reg-event-info">
                        ...
                        <div className="reg-event-meta">
                          <CalendarDays size={12} /> {fmtDate} &nbsp;{" "}
                          <MapPin size={12} /> {ev.location || "미정"}
                        </div>
                      </div>
                      <span className="reg-event-badge">
                        {ev.baseFee && Number(ev.baseFee) > 0
                          ? `₩${Number(ev.baseFee).toLocaleString()}`
                          : "무료"}
                      </span>
                    </div>
                  );
        ...
        <>
          ...
          <div className="reg-card">
            ...
            <div className="reg-summary">
              <div className="reg-summary-row">
                <span>행사명</span>
                <span>{selectedEventObj?.eventName || "—"}</span>
              </div>
              <div className="reg-summary-row">
                <span>티켓 종류</span>
          ...
          <div className="reg-btn-row">
            ...
            <button
              className="reg-btn reg-btn-primary"
              style={{
                opacity: agrees[0] && agrees[1] && !submitting ? 1 : 0.4,
              }}
              onClick={handleApply}
              disabled={submitting}
            >
              {submitting ? "신청 중..." : "결제하기"}{" "}
        ...
        <div className="reg-card">
          <div className="reg-success">
            ...
            <div className="reg-success-desc">
              행사 참가 신청이 정상적으로 접수되었습니다.
              <br />
              관리자 승인 후 참가가 확정됩니다.
            </div>
            {applyResult && (
              <div className="reg-success-num">
                신청번호: #{applyResult.applyId}
              </div>
            )}
            <div
              style={{
