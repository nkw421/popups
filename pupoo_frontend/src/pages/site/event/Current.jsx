...
const styles = `
  ...
  .ev-card-btn {
    height: 32px; padding: 0 14px; border: 1px solid #e2e8f0; border-radius: 8px;
...
const SUBTITLE_MAP = {
  "/events/current": "현재 진행 중인 행사 목록을 확인합니다",
};
...
export default function Current() {
  ...
  const [currentPath] = useState("/events/current");
      ...
      <header className="ev-header">
        <div className="ev-header-inner">
          <div className="ev-header-left">
            <div className="ev-header-title">행사 관리</div>
            <div className="ev-header-sub">{SUBTITLE_MAP[currentPath]}</div>
          </div>
          <nav className="ev-nav">
            {NAV_ITEMS.map((n) => (
              <button
                key={n.path}
                className={`ev-nav-btn${currentPath === n.path ? " active" : ""}`}
              >
                {n.label}
              </button>
            ))}
