...
function normalizeStatus(status) {
  ...
  if (value === "ended" || value === "end" || value === "closed")
    return "ended";
  return "upcoming";
}
      ...
      <div className="ev-container">
        ...
        {filtered.length > 0 ? (
          <div className="ev-grid">
            {filtered.map((event) => {
              const color = event?.color || "#7CB342";
              const status = normalizeStatus(event?.status);
              const thumbnail = event?.imageUrl || event?.thumbnail;
              const participants = Number(event?.participants || 0);

              const isEnded = status === "ended";
              const isHovered = !isEnded && hoveredId === event.id;
              return (
                <div
                  key={event.id}
                  className={`ev-card${isEnded ? " ev-card-ended" : ""}`}
                  style={{
                    ...(isEnded ? { pointerEvents: "none" } : {}),
                    ...(isHovered
                      ? {
                          borderColor: "#b4c6f0",
                          transform: "translateY(-3px)",
                          boxShadow: "0 8px 28px rgba(26, 79, 214, 0.08)",
                        }
                      : {}),
                  }}
                  onMouseEnter={() => !isEnded && setHoveredId(event.id)}
                  onMouseLeave={() => setHoveredId(null)}
                  onClick={() => !isEnded && handleSelect(event)}
